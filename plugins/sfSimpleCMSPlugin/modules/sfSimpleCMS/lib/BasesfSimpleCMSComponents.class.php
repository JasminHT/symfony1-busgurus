...
class BasesfSimpleCMSComponents extends sfComponents
{
  public function executeEditorTools()
  {
    ...
    if (is_null($this->edit_form))
    {
      ...
      $this->edit_form->setDefault('title',$this->page->getTitle($this->culture));
      $this->edit_form->setDefault('slug',$this->page->getSlug());
      $this->edit_form->setDefault('template',$this->page->getTemplate());
      $this->edit_form->setDefault('position',$this->slug);
    }
    if (is_null($this->create_form))
    {
